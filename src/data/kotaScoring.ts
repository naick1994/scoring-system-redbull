...
/** The WindGames table tiers, with the name they gave each score level. */
...
export function binaryPoints(score: number, threshold: number) {
...
export function steppedPoints(score: number, threshold: number) {
  if (score < threshold) return 0;
  for (const [min, pts] of STEPS) if (score >= min) return pts;
  return 0;
}

/** The WindGames tier name for a score (independent of the proposal's threshold). */
export function tierLabel(score: number): string {
  for (const [min, label] of TIERS) if (score >= min) return label;
  return '–';
}
...
export const EXAMPLE_SCORES = [5.0, 6.0, 6.5, 7.0, 7.5, 8.0, 8.5, 9.0, 9.5];
...
export const PROBLEMS = [
  ...
  'A slightly stricter score, if it drops the trick below threshold, removes a full point: a judge\'s error carries the maximum possible weight, with no middle ground.',
...
export const PROPOSALS: Proposal[] = [
  {
    ...
    autoImpMax: 7,
    impressionMax: 3,
    title: 'Top 3 jumps + progressive Auto Impression from a score of 5.0',
    note: 'The proposal closest to today: same 3 jumps, no new rules to learn. Only the Auto Impression becomes progressive (no longer all or nothing) and starts counting from a minimum score of 5.0 instead of 6.5.',
    proposedBy: 'Marijn',
    solves: [
      { id: 1, how: 'Above threshold, points grow with the score instead of always being 1: a trick scored 9 is worth more than one scored 6.5.' },
      { id: 3, how: 'Being fixed, the threshold stays the same in every heat, from the start of the event to the end.' },
      { id: 4, how: 'A slightly stricter score shifts the trick down one tier in the table, it doesn\'t zero it out entirely (unless it drops below 5.0).' },
    ],
  ...
  },
  {
    ...
    autoImpMax: 7,
    impressionMax: 3,
    title: 'Top 5 jumps + progressive Auto Impression from a score of 5.0',
  ...
  },
  {
    ...
    solves: [
      { id: 0, how: 'There\'s no more score threshold for the bonus: every jump is worth its full score from 0 to 10, whatever it is.' },
      ...
      { id: 4, how: 'A slightly stricter score changes the jump\'s total by a little, it doesn\'t zero it out.' },
    ...
    tradeoff: 'There\'s no more score threshold, but a new cliff-edge appears: a trick in a category already landed is worth zero, no matter how well it\'s executed.',
  },
  {
    id: '4',
    kind: 'unique-only',
    ...
    whyRecommended: [
      {
        ...
        text: 'No threshold, no separate Auto Impression, no hidden math: a jump either lands in a new category or it doesn\'t, and its score is exactly what the judges give it. Riders, judges and spectators can all follow it live, without a rulebook.',
      },
      {
