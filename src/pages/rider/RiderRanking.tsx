    ...
    <>
      ...
      <Card className="overflow-hidden shadow-[var(--shadow-card)]">
        <table className="w-full border-collapse">
          ...
          <tbody>
            {event.standings.map((row, idx) => {
              const isMe = row.athlete === RIDER_NAME;
              const country = getCountryForAthlete(row.athlete);
              return (
                <tr
                  key={idx}
                ...
                >
                  ...
                  <td className="py-3 px-4 text-2xl" title={country}>{COUNTRY_FLAGS[country] ?? country}</td>
                  <td className="py-3 px-4 text-right font-semibold">{row.points}</td>
                </tr>
              );
            })}
